...
<body>
    ...
    <p id="logo">Macarena<span id="highlight">.</span></p>


    <?php
    $langue = $_SESSION['langue'];

    // Récupérer les données du quartier Macarena depuis la base de données
    $query = $cnx->prepare("SELECT * FROM seville.macarena WHERE language = :langue or type = 'image'");
    ...
    <table>
        ...
        <?php
        $query = $cnx->prepare("SELECT * FROM seville.macarena");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $monuments = [];
        $barsRestaurants = [];
