...
<body>
    ...
    <p id="logo">Santa Cruz<span id="highlight">.</span></p>


    <?php
    $langue = $_SESSION['langue'];

    // Récupérer les données du quartier Santa Cruz depuis la base de données
    $query = $cnx->prepare("SELECT * FROM seville.santa_cruz WHERE language = :langue or type = 'image'");
    ...
    <table>
        ...
        <?php
        $query = $cnx->prepare("SELECT * FROM seville.santa_cruz");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $monuments = [];
        $barsRestaurants = [];
