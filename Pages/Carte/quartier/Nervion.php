...
<body>
    ...
    <p id="logo">Nervión<span id="highlight">.</span></p>


    <?php
    $langue = $_SESSION['langue'];

    // Récupérer les données du quartier Nervion depuis la base de données
    $query = $cnx->prepare("SELECT * FROM seville.nervion WHERE language = :langue or type = 'image'");
    ...
    <table>
        ...
        <?php
        $query = $cnx->prepare("SELECT * FROM seville.nervion");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $monuments = [];
        $barsRestaurants = [];
