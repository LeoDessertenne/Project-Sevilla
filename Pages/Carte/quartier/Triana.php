...
<body>
    ...
    <p id="logo">Triana<span id="highlight">.</span></p>


    <?php
    $langue = $_SESSION['langue'];

    // Récupérer les données du quartier Triana depuis la base de données
    $query = $cnx->prepare("SELECT * FROM seville.triana WHERE language = :langue or type = 'image'");
    ...
    <table>
        ...
        <?php
        $query = $cnx->prepare("SELECT * FROM seville.triana");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $monuments = [];
        $barsRestaurants = [];
