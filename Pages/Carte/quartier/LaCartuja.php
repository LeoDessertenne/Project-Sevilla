...
<body>
    ...
    <p id="logo">La Cartuja<span id="highlight">.</span></p>


    <?php
    $langue = $_SESSION['langue'];

    // Récupérer les données du quartier La Cartuja depuis la base de données
    $query = $cnx->prepare("SELECT * FROM seville.la_cartuja WHERE language = :langue or type = 'image'");
    ...
    <table>
        ...
        <?php
        $query = $cnx->prepare("SELECT * FROM seville.la_cartuja");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $monuments = [];
        $barsRestaurants = [];
